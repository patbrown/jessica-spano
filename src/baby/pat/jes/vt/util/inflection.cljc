(ns baby.pat.jes.vt.util.inflection
  (:refer-clojure :exclude [replace])
  (:require [clojure.string :refer [blank? lower-case replace]]))
...
(defn add-rule! [rules rule]
  (when-not (contains? (set (deref rules)) rule)
...
(defn ordinality-of
  ...
  [s]
  (when (string? s) (if (and (plural? s) (not (singular? s)))
                      :plural
                      (when (and (singular? s) (not (plural? s))) :singular))))
...
(defn ordinalize
  ...
  Examples:
    ...
    ;=> \"23rd\""
  [x]
  (when-let [number #?(:clj (Integer/parseInt x)
                     :cljs (js/parseInt x))]
...
(defn cardinalize-keyword [f]
  (fn [kw] (when (keyword? kw)
             (if (qualified-keyword? kw)
               (keyword (namespace kw) (f (name kw)))
               (keyword (f (name kw)))))))

(def plural-keyword (cardinalize-keyword plural))
(defn plural-keyword?
  "Returns true if calling singular on a value returns the same value"
  [s]
  (= s (plural-keyword s)))
(def singular-keyword (cardinalize-keyword singular))
(defn singular-keyword?
  "Returns true if calling singular on a value returns the same value"
  [s]
  (= s (singular-keyword s)))
